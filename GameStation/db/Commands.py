...
class SQLCommand:
    ...
    GERENCIAR_JOGOS = "SELECT jogo_aceito.id AS id_jogo FROM jogo_pendente JOIN jogo_aceito ON (jogo_aceito.id=jogo_pendente.id) WHERE(jogo_pendente.id_dev=:id_usuario AND jogo_pendente.id NOT IN (SELECT id_jogo FROM jogo_removido));"
